...
fn setup_test_graph() -> (Graph, String, String, String) {
    ...
    // 高分節點
    let high = Node::new_with("高分".to_string(), 1, "高分".to_string(), 0.9, 0.9, 10.0);
    let high_id = high.id.clone();

    // 中分節點
    let mid = Node::new_with("中分".to_string(), 1, "中分".to_string(), 0.5, 0.5, 5.0);
    let mid_id = mid.id.clone();

    // 低分節點
    let low = Node::new_with("低分".to_string(), 1, "低分".to_string(), 0.1, 0.1, 2.0);
...
fn test_converge_preserves_locked() {
    let mut graph = Graph::new();
    let mut node = Node::new_with("鎖定".to_string(), 1, "鎖定".to_string(), 0.1, 0.1, 1.0);
...
fn test_converge_smart() {
    let mut graph = Graph::new();
    let node = Node::new_with("高信心".to_string(), 1, "高信心".to_string(), 0.9, 0.9, 10.0);
