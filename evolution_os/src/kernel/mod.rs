...
impl Kernel {
    ...
    pub fn syscall(&mut self, call: SysCall) -> SysCallResult {
        ...
        match call.kind {
            ...
            SysCallKind::Send { target, content } => self.do_send(call.caller, target, content),

            SysCallKind::Receive => self.do_receive(call.caller),

            SysCallKind::Wait { target } => self.do_wait(call.caller, target),

            SysCallKind::Exit { code } => self.do_exit(call.caller, code),
        }
    }
    ...
    fn do_wait(&mut self, caller: Pid, target: Pid) -> SysCallResult {
        // 先檢查目標狀態（不可變借用）
        let target_done = match self.table.state(target) {
            Some(state) => state == ProcessState::Done,
            None => return SysCallResult::err(&format!("wait: target {} not found", target.value())),
        };
    ...
    pub fn schedule(&mut self) -> Option<Pid> {
        ...
        // 更新 valid_pids 並喚醒等待中的行程
        self.scheduler.sync_valid_pids(&self.table);
        self.scheduler.update(&mut self.table);

        // 取下一個
        let next = self.scheduler.next(&mut self.table);
...
mod tests {
    ...
    fn test_kernel_send_and_receive() {
        ...
        let planner = k
            .syscall(make_spawn(SysCallKind::Spawn {
                name: "planner".into(),
                system_prompt: "planner".into(),
            }))
            .value;
        let planner_pid = match planner {
            ResultValue::Pid(p) => p,
            _ => panic!(),
        };
