...
mod tests {
    ...
    fn create_test_graph() -> Graph {
        ...
        // 建立多個節點，有些高分有些低分
        let node1 = Node::new_with("高分節點".to_string(), 1, 0.9, 0.9, 10.0);
        let node2 = Node::new_with("低分節點".to_string(), 1, 0.1, 0.1, 5.0);
        let node3 = Node::new_with("中分節點".to_string(), 1, 0.5, 0.5, 8.0);
    ...
    fn test_converge_does_not_remove_locked() {
        let mut graph = Graph::new();
        let mut node = Node::new_with("鎖定節點".to_string(), 1, 0.1, 0.1, 1.0);
