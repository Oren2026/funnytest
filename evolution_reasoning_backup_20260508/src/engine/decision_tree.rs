...
mod tests {
    ...
    fn test_decision_node_from_node() {
        ...
        let root = Node::new_with("根節點".to_string(), 1, 0.8, 0.9, 1.0);
        let root_id = root.id.clone();
        graph.add_node(root);

        let child = Node::new_with("子節點".to_string(), 2, 0.7, 0.8, 1.0);
    ...
    fn test_decision_tree_from_graph() {
        let mut graph = Graph::new();
        let root = Node::new_with("測試主題".to_string(), 1, 0.8, 0.9, 1.0);
        graph.add_node(root);
    ...
    fn test_decision_tree_to_markdown() {
        let mut graph = Graph::new();
        let root = Node::new_with("睡眠改善".to_string(), 1, 0.8, 0.9, 1.0);
        let root_id = root.id.clone();
        graph.add_node(root);

        // 加入方向 A
        let dir_a = Node::new_with("方向 A：睡眠衛生".to_string(), 2, 0.7, 0.85, 1.0);
        let dir_a_id = dir_a.id.clone();
        graph.add_node(dir_a);
        graph.add_edge(Edge::new(root_id.clone(), dir_a_id.clone(), EdgeType::Reasoning));

        let leaf_a1 = Node::new_with("光線控制".to_string(), 3, 0.6, 0.9, 0.5);
        ...
        // 加入方向 B
        let dir_b = Node::new_with("方向 B：生理調節".to_string(), 2, 0.8, 0.8, 1.0);
