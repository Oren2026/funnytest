...
mod tests {
    ...
    fn test_backtrack_manager_create_checkpoint() {
        ...
        let node = crate::models::Node::new_with("test".to_string(), 1, "test".to_string(), 0.8, 0.9, 1.0);
...
fn graph_from_xml(xml: &str) -> Result<Graph, String> {
    ...
    for node_chunk in node_section.split("<node ").skip(1) {
        ...
        let mut node = Node::new_with(content.clone(), step, content.clone(), weight, confidence, 0.0);
        node.id = id;
        node.status = status;
        graph.add_node(node);
    }
