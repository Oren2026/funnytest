...
impl Graph {
    ...
    /// 取得有 Feedback 的節點摘要（v0.7 新增）
    ///
    /// 用於 gemma4 了解哪些節點已有執行結果。
    pub fn feedback_summary(&self) -> String {
        let nodes_with_feedback: Vec<&Node> = self
            .nodes
            .values()
            .filter(|n| n.feedback_result.is_some())
            .collect();

        if nodes_with_feedback.is_empty() {
            return "（尚無 Feedback 記錄）".to_string();
        }

        let lines: Vec<String> = nodes_with_feedback
            .iter()
            .map(|n| {
                let status_icon = match n.status {
                    NodeStatus::Failed => "[失敗]",
                    _ => "[成功]",
                };
                let feedback = n.feedback_result.as_ref().unwrap();
                let preview = feedback.chars().take(80).collect::<String>();
                let timestamp = n.feedback_timestamp
                ...
                format!(
                    ...
                    n.content.chars().take(20).collect::<String>(),
                    timestamp,
                    preview
                )
            })
        ...
        format!("Feedback 記錄（{} 筆）:\n{}", nodes_with_feedback.len(), lines.join("\n"))
    }
}
...
mod tests {
    ...
    fn test_graph_total_complexity() {
        let mut graph = Graph::new();
        let node1 = Node::new_with("節點1".to_string(), 1, "節點1".to_string(), 1.0, 1.0, 10.0);
        let node2 = Node::new_with("節點2".to_string(), 2, "節點2".to_string(), 1.0, 1.0, 20.0);
    ...
    fn test_graph_find_low_score_nodes() {
        let mut graph = Graph::new();
        let node1 = Node::new_with("節點1".to_string(), 1, "節點1".to_string(), 0.9, 0.9, 0.0); // score = 0.81
        let node2 = Node::new_with("節點2".to_string(), 2, "節點2".to_string(), 0.1, 0.1, 0.0); // score = 0.01
